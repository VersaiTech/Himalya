...
// Process incoming webhook from the payment gateway
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Capture and sanitize incoming POST data
    $member_user_id = sanitize_input($_POST["member_user_id"], $conn);
    $payment_amount = sanitize_input($_POST["payment_amount"], $conn);
    $payment_status = sanitize_input($_POST["payment_status"], $conn); // Expected to be "success", "failure", etc.
    $payment_time = date("Y-m-d H:i:s");
    
    // Generate a unique client transaction ID
    $client_txn_id = generateUniqueClientTxnID();

    // Validate payment status (usually webhook sends 'success' or 'failed')
    if ($payment_status === 'success') {
        // Update the member's topUp_status in the database
        $updateStmt = $conn->prepare("UPDATE tbl_memberreg SET topUp_status = 1 WHERE member_user_id = ?");
        $updateStmt->bind_param("i", $member_user_id);
        $updateStmt->execute();
        ...
        $insertStmt->bind_param("isds", $member_user_id, $payment_time, $payment_amount, $payment_status);
        $insertStmt->execute();

        // Give referral bonuses if applicable
        giveReferralBonus($conn, $member_user_id, $payment_time, $payment_amount);

        // Close the prepared statements
        $updateStmt->close();
        $insertStmt->close();
        
        // Send success response to the payment gateway
        ...
        echo json_encode(["status" => false, "message" => "Payment failed or invalid"]);
    }
} else {
