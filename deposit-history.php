...
// Retrieve the current session's member user ID
...
<body>
    <main class="hp-bg-color-dark-90 d-flex min-vh-100">
                        ...
                        <div class="card custom-card">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-center flex-wrap mb-3">
                                    <h3 class="card-title mb-15">Deposit History</h3>
                                    ...
                                    <table class="table table-hover" style="text-align: center;">
                                        <thead>
                                            <tr>
                                                ...
                                                <th scope="col" style="min-width: 170px;">EMAIL ID</th>
                                                <th scope="col" style="min-width: 120px;">AMOUNT</th>
                                                <th scope="col" style="min-width: 120px;">PACKAGE TYPE</th>
                                                <th scope="col" style="min-width: 120px;">SCREENSHOT</th>
                                        ...
                                        <tbody id="tableBody">
                                            <?php
                                            $cnt = 1;
                                            // Adjust your SQL query to filter by the current session's member user ID
                                            $str = "SELECT * FROM deposit_request WHERE member_user_id = '$member_user_id'";
                                            $rop = mysqli_query($connection, $str);

                                            if (!$rop) {
                                                die("Query Failed: " . mysqli_error($connection));
                                            }

                                            while ($rjk = mysqli_fetch_array($rop)) {
                                            ?>
                                                <tr>
                                                    <td>
                                                        <button type="button" class="btn btn-danger ml-15" data-toggle="modal" data-target="#deleteModal" data-member-id="<?php echo $rjk['member_user_id']; ?>" data-transaction-id="<?php echo $rjk['transaction_id']; ?>" style="margin-left: 10px;">
                                                            Delete
                                                        </button>
                                                    </td>
                                                    ...
                                                    <td><?php echo date('Y-m-d', strtotime($rjk['created_at'])); ?></td>
                                                    <td><?php echo $rjk['email_id'] ?></td>
                                                    <td><?php echo $rjk['package_amount'] ?></td>
                                                    <td><?php echo $rjk['package_type'] ?></td>
                                                    <td>
                                                        <!-- Add a modal to display the screenshot -->
                                                        <div class="modal fade" id="screenshotModal<?php echo $rjk['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="screenshotModalLabel" aria-hidden="true">
                                                            <div class="modal-dialog" role="document">
                                                                <div class="modal-content">
                                                                    <div class="modal-header">
                                                                    ...
                                                                    <div class="modal-body">
                                                                        ...
                                                                        <img src="ACApp/uploads/<?php echo basename($rjk['payment_screenshot']); ?>" alt="Screenshot" class="img-fluid">
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#screenshotModal<?php echo $rjk['id']; ?>">Show Screenshot</button>
                                                    </td>
                                                    <td> <?php
                                                            $wallet_address = $rjk['wallet_address'];
                                                            $truncated_wallet_address = substr($wallet_address, 0, 4) . '...' . substr($wallet_address, -4);
                                                            echo $truncated_wallet_address;
                                                            ?></td>

                                                    <td><?php echo $rjk['transaction_id'] ?></td>
                                                </tr>
                                            <?php
                                            }
                                            ?>
        ...
        <script>
            $(document).ready(function() {
                var memberId, transactionId;

                // When the delete button is clicked, store the member id and transaction id
                ...
                // When the confirm delete button is clicked, send an AJAX request to delete the entry
                $('#confirmDelete').on('click', function() {
                    console.log('Sending request to delete with Member ID:', memberId, 'and Transaction ID:', transactionId);
                    $.ajax({
                        url: 'delete_entry', // URL to your PHP file
                        type: 'POST',
                        data: {
                        ...
                        success: function(response) {
                            ...
                            if (response.trim() === 'success') {
                                // If success, close the modal and refresh or update the UI as needed
                                $('#deleteModal').modal('hide');
                                location.reload(); // Reload the page to see the changes
                            } else {
                                // Handle error
                                alert('Failed to delete the entry: ' + response);
                            }
                        },
    ...
    <?php
    include "components/jsComponents/deposit2-js.php"
    ?>
