...
$date=date("Y-m-d");
 $sys_date=date('Y-m-d');
echo $RStr="Select * from tbl_reinvest where cal_team=0 and status=1 and payment_status=1 order by record_no limit 0,2";
...
while($rowR=mysqli_fetch_array($resR))
{
    ...
    $investment_busd=$rowR['investment_busd'];
     ...
     $upL="Update tbl_reinvest set cal_team=1 where record_no=$record_no";
...
function cal_Team($topup_amount,$member_user_id,$invest_aura)
{
    ...
    for($cnt=1;$cnt<=$total_level;$cnt++)
    {
		...
		while($mrow=mysqli_fetch_array($mresult))
		{
           ...
           echo "<br>". $up="Update tbl_memberreg set team_business=team_business+$topup_amount,team_business_aura=team_business_aura+$invest_aura where member_user_id='$sponcer_id'";
